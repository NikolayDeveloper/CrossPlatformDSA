...
namespace CrossPlatformDSA.DSA.Models
{
    public class ECPServiceLinux : IECPService
    {
       ...
       // outInfoLength=LENGTH;
        ...
        public bool VerifyData(byte[] cms,out UserCertInfo userCertInfo)
        {
            userCertInfo = null;
            ulong codeError;
            ...
            byte[] outVerifyInfo = new byte[2000];
            ...
            catch (Exception ex)
            {
                ...
                userCertInfo.extraInfo = ex.Message+" InnerExeption: " + ex.InnerException;
            }
            return res;
        }
        ...
        {
            byte[] errStr = new byte[LENGTH];
            byte[] outCertVerifyData = new byte[LENGTH];
            byte[] outData = new byte[cms.Length];
            int outCertVerifyDataLength = LENGTH;
            byte[] outVerifyInfo2 = new byte[5000];
            int outVerifyInfoLen2 = LENGTH;
            int outDataLen = outData.Length;
            ...
            int kalkanFlag = (int)KalkanCryptCOMLib.KALKANCRYPTCOM_FLAGS.KC_SIGN_CMS +
                ...
                (int)KalkanCryptCOMLib.KALKANCRYPTCOM_FLAGS.KC_OUT_BASE64 +
                (int)KalkanCryptCOMLib.KALKANCRYPTCOM_FLAGS.KC_WITH_TIMESTAMP;
            byte[] dataRandom = { 100, 97, 116, 97 };// просто требуется передать какую нибудь строку
            ulong codeError = KC_GetLastErrorString(ref errStr[0], ref bufSize);
            if (codeError != 0)
            {
                throw new Exception("Допущена ошибка до вызова метода KC_GetLastErrorString, код ошибки: " + codeError.ConvertToHexError(),
                                    new Exception(Encoding.UTF8.GetString(errStr)));
            }
            // использую метод VerifyData, чтобы вытащить сертификат. Также можно использовать метод GetCertFromCMS
            codeError = VerifyData("", kalkanFlag, ref dataRandom[0], dataRandom.Length, out cms[0], cms.Length,
                                    out outData[0], out outDataLen, out outVerifyInfo2[0], out outVerifyInfoLen2,
                                    inCertID, out outCertVerifyData[0], out outCertVerifyDataLength);
            if (codeError != 0)
            {
                bufSize = 1000;
                KC_GetLastErrorString(ref errStr[0], ref bufSize);
            ...
            res = Encoding.UTF8.GetString(outCertVerifyData);
            return res;
        }
